...
Autor: Manus AI
Versión: 2.1 (Corrección de importación de CrewAI)
"""
...
# Variable global para verificar la disponibilidad de CrewAI
...
class PluginGenerator(QThread):
    ...
    def run(self):
        global CREWAI_AVAILABLE
        # Importaciones de CrewAI (lazy loading)
        try:
            from crewai import Agent, Task, Crew
            from crewai.llm import LLM
            CREWAI_AVAILABLE = True
        except ImportError as e:
            self.error.emit(f"CrewAI o sus dependencias no están disponibles. Error: {str(e)}\nPor favor, asegúrate de instalar crewai y sus dependencias (como onnxruntime y chromadb) correctamente.")
            CREWAI_AVAILABLE = False
            return

        if not CREWAI_AVAILABLE:
            self.error.emit("CrewAI no está disponible. La generación de plugins estará deshabilitada.")
            return
...
class QGISPluginGenerator(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("Generador de Scripts para Plugins de QGIS v2.1")
    ...
    def create_main_tab(self):
        ...
        # La disponibilidad de CrewAI se verifica en tiempo de ejecución al hacer clic
...
=== Generador de Scripts para Plugins de QGIS v2.1 ===
</div>
...
<hr style=\'border: 1px solid #555555;\'>
        ...
        # Actualizar el estado del botón de generar al iniciar
    ...
    def generate_plugin(self):
        ...
        # Verificar la disponibilidad de CrewAI justo antes de usarla
        if not CREWAI_AVAILABLE:
            self.show_error("CrewAI o sus dependencias no están disponibles. Por favor, asegúrate de instalar crewai y sus dependencias (como onnxruntime y chromadb) correctamente.")
            return
